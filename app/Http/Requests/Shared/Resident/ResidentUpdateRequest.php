<?php

namespace App\Http\Requests\Shared\Resident;

use Illuminate\Validation\Rule;
use Illuminate\Foundation\Http\FormRequest;

class ResidentUpdateRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            'fname' => 'required|sometimes',
            'mname' => 'required|sometimes',
            'lname' => 'required|sometimes',
            'suffix' => 'required|sometimes',
            'gender' => 'required|sometimes',
            'birthdate' => 'required|sometimes',
            'address' => 'required|sometimes',
            'barangay_id' => 'required|sometimes',
            'contact' => 'required|sometimes',
            'email' => ['required', 'sometimes', 'email', Rule::unique('residents')->ignore($this->resident)],
        ];
    }
}
