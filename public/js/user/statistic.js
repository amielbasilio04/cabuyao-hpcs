$(() => {
    // router
    if (window.location.href === route("brgy_admin.health_statistic.index")) {
        getDefaultStatistic();
    }
});
...
async function getDefaultStatistic() {
    const res = await axios.get(route("brgy_admin.health_statistic.index"));
    if (typeof map === "object") {
        map.remove(); // every time we load the data we destroy the instance of the map object
        getStatistic(res.data.results, "Health Issue");
    } else {
        getStatistic(res.data.results, "Health Issue");
    }
}
...
async function getStatisticByHealthIssue(health_issue) {
    if (health_issue.value) {
        ...
        const res = await axios.get(
            route("brgy_admin.health_statistic.index"),
            {
